...
export function Services() {
  const [active, setActive] = useState<ServiceTabKey>("all");

  const filtered = useMemo(() => {
    if (active === "all") return services;
    return services.filter((s) => s.tabKey === active);
  }, [active]);
...
function ServiceCard({ service }: { service: Service }) {
  ...
  const priceLabel = service.priceLabel ?? "시작가";

  if (featured) {
    return (
      <article className="group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-2xl border border-ink-90 bg-ink-100 p-6 transition-[transform,border-color] duration-200 hover:-translate-y-0.5 hover:border-ink-70">
  ...
  return (
    <article className="group relative flex h-full cursor-pointer flex-col rounded-2xl border border-ink-15 bg-ink-5 p-6 transition-[background,border-color,transform] duration-200 hover:-translate-y-0.5 hover:border-iris hover:bg-white">
