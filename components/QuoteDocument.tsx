...
/**
 * Printable A4 quotation document. Pure presentational — shared by the admin
 * print route and the customer self-serve print route so both render an
 * identical PDF. Wrap with a print toolbar (client) at the call site.
 */
        ...
        <article className="print-page rounded-2xl border border-ink-15 bg-white p-10">
          ...
          <section className="mt-7 grid grid-cols-2 gap-6 text-[12.5px]">
            <div>
              ...
              {customer.company && customer.company !== customer.name ? (
                <p className="text-[12px] text-ink-70">{customer.company}</p>
              ) : null}
              <p className="mt-1 text-[12px] text-ink-70">
                {customer.email || "—"}
              </p>
              <p className="text-[12px] text-ink-70">{customer.phone || "—"}</p>
            </div>
            <div className="text-right">
              <p className="font-display text-[10px] font-bold uppercase tracking-[0.12em] text-ink-50">
                발행일
          ...
          <section className="mt-7">
            <table className="w-full border-t border-b border-ink-100 text-[13px]">
              ...
              <tbody className="divide-y divide-ink-15">
                ...
                {options.map((o, i) => (
                  <tr key={i}>
                    <td className="py-3 pr-4 text-ink-70">옵션 · {o.label}</td>
                    <td className="num py-3 text-right text-ink-100">
                      +{fmt(o.price)}원
                    </td>
                  </tr>
          ...
          <section className="mt-7 grid grid-cols-2 gap-4 rounded-2xl border border-ink-15 bg-ink-5 p-5">
            <div>
              ...
              <p className="mt-1 text-[11px] text-ink-50">착수 전 예약금 입금</p>
            </div>
            <div>
              <p className="font-display text-[10px] font-bold uppercase tracking-[0.12em] text-ink-50">
                잔금
              ...
              <p className="mt-1 text-[11px] text-ink-50">최종 검수 후 본결제</p>
            </div>
          </section>
          ...
          <section className="mt-8 border-t border-ink-15 pt-6 text-[11.5px] leading-[1.7] text-ink-70">
            ...
            <ul className="mt-2 space-y-1.5 pl-4 [list-style:disc]">
              ...
              <li>
                본 견적서는 발행일 기준{" "}
                {q.expires_at ? "유효기간 표기일" : "30일"}까지 유효합니다.
              </li>
              ...
              <li>
                ...
                <span className="underline">
                  {siteUrl.replace(/^https?:\/\//, "")}/legal/terms
                </span>{" "}
                에서 확인하실 수 있습니다.
              </li>
            </ul>
